...
def recommend_travel_places(
    ...
    sido_map_path: str = None,
    sigungu_map_path: str = None,
    sido_names_path: str = None,
    feature_cols_path: str = None,
) -> pd.DataFrame:
    
    # 현재 recommend_travel_places.py 의 절대경로
    base_dir = os.path.dirname(os.path.abspath(__file__))
    
    # 경로를 디폴트로 재설정
    if model_path is None:
        model_path = os.path.join(base_dir, "catboost_travel_satisfaction_model.joblib")
    if encoders_path is None:
        encoders_path = os.path.join(base_dir, "label_encoders_for_catboost.joblib")
    if sido_map_path is None:
        sido_map_path = os.path.join(base_dir, "area_to_sido_map.joblib")
    if sigungu_map_path is None:
        sigungu_map_path = os.path.join(base_dir, "area_to_sigungu_map.joblib")
    if sido_names_path is None:
        sido_names_path = os.path.join(base_dir, "sido_change_names.json")
    if feature_cols_path is None:
        feature_cols_path = os.path.join(base_dir, "model_feature_columns.joblib")
